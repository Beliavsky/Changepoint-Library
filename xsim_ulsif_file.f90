program xsim_ulsif_file
! read a univariate series from a text file and score it with a deterministic Gaussian-kernel ULSIF detector
...
call get_data_file_arg("xulsif_data.txt", data_file)
call read_series_file(data_file, x)
allocate(score(size(x)))
call solve_rulsif_gaussian_1d(x, window_length, n_windows, lag, 0.0_dp, sigma, lambda_reg, score, cp, best_score, scoring_step)
