"""
Simulate a univariate series and score the raw roerich SlidingWindows energy detector.
"""
...
def fit_sliding_windows_energy(
    signal: np.ndarray,
    window_size: int,
    periods: int = 1,
    step: int = 1,
    ...
    """
    Run the raw SlidingWindows energy score and return the best shifted changepoint.
    """
    detector = SlidingWindows(
        metric="energy",
        bootstrap=False,
        periods=periods,
...
def main() -> None:
    ...
    signal = simulate_roerich_signal(n, true_cp, means, sds, 101)
    cp, score = fit_sliding_windows_energy(signal, window_size, periods, step)
