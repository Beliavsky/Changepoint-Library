program xsim_beast_irreg_file
use kind_mod, only: dp, long_int
use util_mod, only: print_wall_time
use compare_io_mod, only: get_data_file_arg, read_time_series_file
...
real(kind=dp), allocatable :: t(:), y(:), cp_prob(:), fitted_mean(:)
real(kind=dp) :: model_weights(max_cp + 1), top_prob(ntop)

call get_data_file_arg('xbeast_irreg_data.txt', data_file)
call read_time_series_file(trim(data_file), t, y)
n = size(y)
allocate(cp_prob(n), fitted_mean(n))
...
deallocate(t, y, cp_prob, fitted_mean)
end program xsim_beast_irreg_file
