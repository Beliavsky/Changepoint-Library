...
character(len=*), parameter :: draws_file = "xmcp_demo_draws.txt"
real(kind=dp), allocatable :: dat(:, :), draws(:, :), fitted_mean(:), cp_means(:), param_means(:)

call get_data_file_arg("xmcp_demo_data.txt", data_file)
...
end program xsim_mcp_demo_file
