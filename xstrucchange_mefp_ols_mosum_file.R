...
data_file <- if (length(args) >= 1L) args[[1L]] else "xstrucchange_mefp_ols_mosum_data.txt"
history_n <- 50L
alpha <- 0.05
h <- 0.25
...
me0 <- strucchange::mefp(y ~ x, type = "OLS-MOSUM", data = hist_dat, h = h, alpha = alpha)
