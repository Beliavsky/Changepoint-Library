program xsim_dpdu_regression_file
! read regression data from a text file and estimate changepoints by DPDU.regression
use kind_mod, only: dp, long_int
use compare_io_mod, only: read_matrix_file, get_data_file_arg
use changepoints_pkg_dpdu_mod, only: solve_dpdu_regression_1d
...
y = dat(:, 1)
x = dat(:, 2:size(dat, 2))
call solve_dpdu_regression_1d(y, x, lambda, zeta, eps, partition, cpt_hat, beta_mat)
...
end program xsim_dpdu_regression_file
