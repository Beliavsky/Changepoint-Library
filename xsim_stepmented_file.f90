program xsim_stepmented_file
use kind_mod, only: dp
use compare_io_mod, only: read_matrix_file, get_data_file_arg
use segmented_pkg_mod, only: eval_stepmented_lm1_draws
implicit none

character(len=256) :: data_file
character(len=*), parameter :: draws_file = "xstepmented_draws.txt"
real(kind=dp), allocatable :: dat(:, :), draws(:, :), fitted_mean(:), param_means(:)

call get_data_file_arg("xstepmented_data.txt", data_file)
call read_matrix_file(data_file, dat)
call read_matrix_file(draws_file, draws)
call eval_stepmented_lm1_draws(dat(:, 1), draws, fitted_mean, param_means)

print *, "file               =", trim(data_file)
print *, "draws_file         =", draws_file
print *, "n                  =", size(dat, 1)
print *, "psi                =", param_means(1)
print *, "coef checksum      =", sum(param_means(2:3))
...
print *, "jump               =", param_means(3)
end program xsim_stepmented_file
