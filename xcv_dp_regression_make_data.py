#!/usr/bin/env python
"""Write data for changepoints::CV.search.DP.regression comparisons."""
...
def main() -> None:
    data_file = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("xcv_dp_regression_data.txt")
    ...
    with data_file.open("w", encoding="utf-8") as fh:
        fh.write("# y x1 x2 x3 for changepoints::CV.search.DP.regression\n")
        fh.write(f"# n = {n}\n")
        fh.write(f"# p = {p}\n")
