...
data_file <- if (length(args) >= 1L) args[[1L]] else "xsegmented_data.txt"
draws_file <- "xsegmented_draws.txt"

library(segmented)

dat <- read.table(data_file, col.names = c("x", "y"))
fit0 <- lm(y ~ x, data = dat)
fit <- segmented(fit0, seg.Z = ~x, psi = 60)
...
cat("hinge slope =", sprintf("%.15f", unname(co[3])), "\n")
