...
data_file <- if (length(args) >= 1L) args[[1L]] else "xcpm_joint_data.txt"
threshold_file <- "xcpm_joint_thresholds.txt"
batch_threshold_file <- "xcpm_joint_batch_threshold.txt"
ARL0 <- 500
startup <- 20
alpha <- 0.05

x <- scan(data_file, quiet = TRUE, comment.char = "#")
thresholds <- cpm:::loadThresholds("Joint", ARL0, length(x), lambda = NA)
batch_threshold <- cpm::getBatchThreshold("Joint", alpha, length(x), lambda = 0)
...
det <- cpm::detectChangePoint(x, "GLR", ARL0 = ARL0, startup = startup)
proc <- cpm::processStream(x, "GLR", ARL0 = ARL0, startup = startup)
bat <- cpm::detectChangePointBatch(x, "GLR", alpha = alpha)
