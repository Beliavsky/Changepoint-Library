...
data_file <- if (length(args) >= 1L) args[[1L]] else "xcpm_exponential_data.txt"
threshold_file <- "xcpm_exponential_thresholds.txt"
batch_threshold_file <- "xcpm_exponential_batch_threshold.txt"
ARL0 <- 500
startup <- 20
alpha <- 0.05

x <- scan(data_file, quiet = TRUE, comment.char = "#")
thresholds <- cpm:::loadThresholds("Exponential", ARL0, length(x), lambda = NA)
batch_threshold <- cpm::getBatchThreshold("Exponential", alpha, length(x), lambda = 0)
...
det <- cpm::detectChangePoint(x, "Exponential", ARL0 = ARL0, startup = startup)
proc <- cpm::processStream(x, "Exponential", ARL0 = ARL0, startup = startup)
bat <- cpm::detectChangePointBatch(x, "Exponential", alpha = alpha)
elapsed <- proc.time()[["elapsed"]] - t0

cat("file =", data_file, "\n")
cat("n =", length(x), "\n")
cat("type = Exponential\n")
cat("ARL0 =", ARL0, "\n")
cat("startup =", startup, "\n")
cat("alpha =", alpha, "\n")
