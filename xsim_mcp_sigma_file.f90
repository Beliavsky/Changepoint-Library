program xsim_mcp_sigma_file
use kind_mod, only: dp
use compare_io_mod, only: read_matrix_file, get_data_file_arg
use mcp_pkg_mod, only: eval_mcp_sigma_draws
implicit none

character(len=256) :: data_file
character(len=*), parameter :: draws_file = "xmcp_sigma_draws.txt"
real(kind=dp), allocatable :: dat(:, :), draws(:, :), fitted_mean(:), sigma_mean(:), cp_means(:), param_means(:)

call get_data_file_arg("xmcp_sigma_data.txt", data_file)
call read_matrix_file(data_file, dat)
call read_matrix_file(draws_file, draws)
call eval_mcp_sigma_draws(dat(:, 1), draws, fitted_mean, sigma_mean, cp_means, param_means)
...
print *, "sigma param checksum =", param_means(4) + param_means(5) + param_means(6)
end program xsim_mcp_sigma_file
