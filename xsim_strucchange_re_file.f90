program xsim_strucchange_re_file
use kind_mod, only: dp, long_int
use compare_io_mod, only: read_matrix_file, get_data_file_arg
use strucchange_pkg_mod, only: solve_efp_re_1d
...
call get_data_file_arg("xstrucchange_re_data.txt", data_file)
call read_matrix_file(data_file, dat)
allocate(y(size(dat, 1)), x(size(dat, 1), size(dat, 2) - 1))
y = dat(:, 1)
x = dat(:, 2:size(dat, 2))

call solve_efp_re_1d(y, x, process)
...
end program xsim_strucchange_re_file
