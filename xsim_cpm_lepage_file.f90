program xsim_cpm_lepage_file
use kind_mod, only: dp, long_int
use compare_io_mod, only: read_series_file, get_data_file_arg
use cpm_pkg_mod, only: solve_cpm_detect_lepage_1d, solve_cpm_process_lepage_1d, solve_cpm_batch_lepage_1d
...
call get_data_file_arg("xcpm_lepage_data.txt", data_file)
call read_series_file(data_file, x)
call read_series_file("xcpm_lepage_thresholds.txt", thresholds)
call read_series_file("xcpm_lepage_batch_threshold.txt", batch_threshold_vec)

call solve_cpm_detect_lepage_1d(x, thresholds, startup, ds_detect, cp_detect, dt_detect)
call solve_cpm_process_lepage_1d(x, thresholds, startup, cps, dts)
call solve_cpm_batch_lepage_1d(x, batch_threshold_vec(1), ds_batch, cp_batch)
...
end program xsim_cpm_lepage_file
