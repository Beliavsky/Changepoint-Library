program xsim_strucchange_ols_mosum_file
use kind_mod, only: dp, long_int
use compare_io_mod, only: read_matrix_file, get_data_file_arg
use strucchange_pkg_mod, only: solve_efp_ols_mosum_1d
...
call get_data_file_arg("xstrucchange_ols_mosum_data.txt", data_file)
...
call solve_efp_ols_mosum_1d(y, x, h, process)
imax = maxloc(abs(process))

print *, "file               =", trim(data_file)
print *, "n                  =", size(y)
print *, "p                  =", size(x, 2)
print *, "type               =", "OLS-MOSUM"
print *, "h                  =", h
...
end program xsim_strucchange_ols_mosum_file
