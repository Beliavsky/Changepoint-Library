...
data_file <- if (length(args) >= 1L) args[[1L]] else "xcpm_cvm_data.txt"
threshold_file <- "xcpm_cvm_thresholds.txt"
batch_threshold_file <- "xcpm_cvm_batch_threshold.txt"
ARL0 <- 500
startup <- 20
alpha <- 0.05

x <- scan(data_file, quiet = TRUE, comment.char = "#")
thresholds <- cpm:::loadThresholds("CVM", ARL0, length(x), lambda = NA)
batch_threshold <- cpm::getBatchThreshold("CVM", alpha, length(x), lambda = 0)
...
det <- cpm::detectChangePoint(x, "Cramer-von-Mises", ARL0 = ARL0, startup = startup)
proc <- cpm::processStream(x, "Cramer-von-Mises", ARL0 = ARL0, startup = startup)
bat <- cpm::detectChangePointBatch(x, "Cramer-von-Mises", alpha = alpha)
