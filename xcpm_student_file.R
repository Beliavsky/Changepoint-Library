...
data_file <- if (length(args) >= 1L) args[[1L]] else "xcpm_student_data.txt"
threshold_file <- "xcpm_student_thresholds.txt"
batch_threshold_file <- "xcpm_student_batch_threshold.txt"
ARL0 <- 500
startup <- 20
alpha <- 0.05

x <- scan(data_file, quiet = TRUE, comment.char = "#")
thresholds <- cpm:::loadThresholds("Student", ARL0, length(x), lambda = NA)
batch_threshold <- cpm::getBatchThreshold("Student", alpha, length(x), lambda = 0)
...
det <- cpm::detectChangePoint(x, "Student", ARL0 = ARL0, startup = startup)
proc <- cpm::processStream(x, "Student", ARL0 = ARL0, startup = startup)
bat <- cpm::detectChangePointBatch(x, "Student", alpha = alpha)
