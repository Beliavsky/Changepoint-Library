"""
Create a univariate Gaussian data file for changepoint.Bocpd comparisons.
"""
...
def main() -> None:
    outfile = sys.argv[1] if len(sys.argv) > 1 else "xbocpd_data.txt"
    seed = 21
    n = 600
    lam = 120.0
    true_bkps = [0, 200, 400]
    ...
    signal = simulate_bocpd_signal(n, true_bkps, means, sds, seed)

    with open(outfile, "w", encoding="ascii") as f:
        f.write(f"# seed = {seed}\n")
        f.write(f"# n = {n}\n")
        f.write(f"# lam = {lam:.6f}\n")
