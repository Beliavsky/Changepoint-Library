...
real(kind=dp), allocatable :: y(:), cp_prob(:), fitted_mean(:)
real(kind=dp) :: model_weights(max_cp + 1), top_prob(ntop)

call get_data_file_arg('xbeast_data.txt', data_file)
...
deallocate(y, cp_prob, fitted_mean)
end program xsim_beast_file
