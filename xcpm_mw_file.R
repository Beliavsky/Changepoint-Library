...
data_file <- if (length(args) >= 1L) args[[1L]] else "xcpm_mw_data.txt"
threshold_file <- "xcpm_mw_thresholds.txt"
batch_threshold_file <- "xcpm_mw_batch_threshold.txt"
ARL0 <- 500
startup <- 20
alpha <- 0.05

x <- scan(data_file, quiet = TRUE, comment.char = "#")
thresholds <- cpm:::loadThresholds("MW", ARL0, length(x), lambda = NA)
batch_threshold <- cpm::getBatchThreshold("MW", alpha, length(x), lambda = 0)
...
det <- cpm::detectChangePoint(x, "Mann-Whitney", ARL0 = ARL0, startup = startup)
proc <- cpm::processStream(x, "Mann-Whitney", ARL0 = ARL0, startup = startup)
bat <- cpm::detectChangePointBatch(x, "Mann-Whitney", alpha = alpha)
