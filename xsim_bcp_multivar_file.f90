program xsim_bcp_multivar_file
use kind_mod, only: dp, long_int
use compare_io_mod, only: read_matrix_file, get_data_file_arg
use bcp_pkg_mod, only: solve_bcp_series_2d
...
real(kind=dp), allocatable :: y(:, :), posterior_mean(:, :), posterior_prob(:)
integer, allocatable :: blocks(:), top_idx(:)
real(kind=dp), allocatable :: top_val(:)
integer(kind=long_int) :: t_start

call system_clock(t_start)
call get_data_file_arg("xbcp_multivar_data.txt", data_file)
call read_matrix_file(data_file, y)

call solve_bcp_series_2d(y, burnin, mcmc, p0, w0, wh_seed, posterior_mean, posterior_prob, blocks)
...
end program xsim_bcp_multivar_file
