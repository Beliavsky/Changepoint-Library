program xsim_bocpd
! simulate a univariate Gaussian series and estimate changepoints by BOCPD
...
integer, parameter :: seed = 21 ! RNG seed used for the simulated series
...
real(kind=dp), parameter :: means(nreg) = [0.0_dp, 3.5_dp, -1.5_dp] ! segment means
real(kind=dp), parameter :: sds(nreg) = [1.0_dp, 1.0_dp, 1.6_dp] ! segment standard deviations
