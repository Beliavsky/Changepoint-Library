program xsim_cpm_fet_file
use kind_mod, only: dp, long_int
use compare_io_mod, only: read_series_file, get_data_file_arg
use cpm_pkg_mod, only: solve_cpm_detect_fet_1d, solve_cpm_process_fet_1d, solve_cpm_batch_fet_1d
use util_mod, only: print_wall_time
implicit none

integer, parameter :: startup = 20
real(kind=dp), parameter :: arl0 = 500.0_dp, alpha = 0.05_dp, lambda = 0.3_dp
...
call get_data_file_arg("xcpm_fet_data.txt", data_file)
call read_series_file(data_file, x)
call read_series_file("xcpm_fet_thresholds.txt", thresholds)
call read_series_file("xcpm_fet_batch_threshold.txt", batch_threshold_vec)

call solve_cpm_detect_fet_1d(x, thresholds, startup, lambda, ds_detect, cp_detect, dt_detect)
call solve_cpm_process_fet_1d(x, thresholds, startup, lambda, cps, dts)
call solve_cpm_batch_fet_1d(x, batch_threshold_vec(1), lambda, ds_batch, cp_batch)
