program xsim_strucchange_efp_file
...
call get_data_file_arg("xstrucchange_efp_data.txt", data_file)
call read_matrix_file(data_file, dat)
allocate(y(size(dat, 1)), x(size(dat, 1), size(dat, 2) - 1))
y = dat(:, 1)
x = dat(:, 2:size(dat, 2))

call solve_efp_ols_cusum_1d(y, x, process)
imax = maxloc(abs(process))

print *, "file               =", trim(data_file)
print *, "n                  =", size(y)
print *, "p                  =", size(x, 2)
print *, "type               =", "OLS-CUSUM"
...
end program xsim_strucchange_efp_file
