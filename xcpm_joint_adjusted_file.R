...
data_file <- if (length(args) >= 1L) args[[1L]] else "xcpm_joint_adjusted_data.txt"
threshold_file <- "xcpm_joint_adjusted_thresholds.txt"
batch_threshold_file <- "xcpm_joint_adjusted_batch_threshold.txt"
ARL0 <- 500
startup <- 20
alpha <- 0.05

x <- scan(data_file, quiet = TRUE, comment.char = "#")
thresholds <- cpm:::loadThresholds("JointAdjusted", ARL0, length(x), lambda = NA)
batch_threshold <- cpm::getBatchThreshold("JointAdjusted", alpha, length(x), lambda = 0)
...
det <- cpm::detectChangePoint(x, "JointAdjusted", ARL0 = ARL0, startup = startup)
proc <- cpm::processStream(x, "JointAdjusted", ARL0 = ARL0, startup = startup)
bat <- cpm::detectChangePointBatch(x, "JointAdjusted", alpha = alpha)
elapsed <- proc.time()[["elapsed"]] - t0

cat("file =", data_file, "\n")
cat("n =", length(x), "\n")
cat("type = JointAdjusted\n")
cat("ARL0 =", ARL0, "\n")
cat("startup =", startup, "\n")
cat("alpha =", alpha, "\n")
