program xsim_text_like_pelt_file
! read an embedding-like matrix from a text file and estimate changepoints by RBF PELT
use kind_mod, only: dp, long_int
use changepoint_mod, only: solve_pelt_rbf_mv
use util_mod, only: print_wall_time, read_matrix
implicit none

real(kind=dp), parameter :: pen = 15.0_dp
...
filename = "xtext_like_data.txt"
call get_command_argument(1, filename, status=status)
if (status /= 0 .or. len_trim(filename) == 0) filename = "xtext_like_data.txt"

end subroutine get_data_file

end program xsim_text_like_pelt_file
